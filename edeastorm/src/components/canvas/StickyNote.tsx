...
import { NOTE_COLORS, DEFAULT_NOTE_COLOR, MAX_NOTE_CONTENT_LENGTH } from '@/lib/constants';
...
export function StickyNote({ data, htmlRef, onUpdate, onDelete }: StickyNoteProps) {
  const inputRef = useRef<HTMLElement>(null);
  const contentRef = useRef('');
  const [isHovered, setIsHovered] = useState(false);

  const { selectedNode, setSelectedNode, editableNode, setEditableNode, isDraggingNode } =
    useNodeStore();
      ...
      >
        ...
        >
          <ContentEditable
            ...
            disabled={!isEditing}
            className="w-full text-center text-zinc-900 leading-tight outline-none"
            style={{
              fontFamily: 'var(--font-caveat), cursive',
              fontSize: data.metadata?.textSize ? `${data.metadata.textSize * 1.5}px` : '24px', // Larger font for handwriting
