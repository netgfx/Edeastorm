...
import { Sparkles, Mail, ArrowLeft, ArrowRight, KeyRound } from 'lucide-react';
  ...
  const handleResetRequest = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: `${window.location.origin}/api/auth/confirm?next=/auth/reset-password&type=recovery`,
      });
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="text-center mb-10 relative z-10">
          <Link href="/" className="inline-flex items-center gap-2 group">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-violet-500/20 group-hover:scale-110 transition-transform">
        ...
        <div className="relative z-10 bg-zinc-900/50 border border-zinc-800 rounded-3xl p-8 backdrop-blur-2xl shadow-2xl">
          ...
          <div className="mt-8 text-center">
            <Link href="/auth/signin" className="text-sm text-zinc-400 hover:text-white transition-colors flex items-center justify-center gap-2">
              <ArrowLeft className="w-4 h-4" />
              Back to Sign In
            </Link>
          </div>
