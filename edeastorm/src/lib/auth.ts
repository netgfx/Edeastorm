  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        if (error || !data.user) {
          console.error('Credentials auth error for', credentials.email, ':', error);
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      ...
      try {
        ...
        if (!existingProfile) {
          ...
          const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-4][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(user.id);
          ...
          if (insertError) {
            console.error('Error creating profile in signIn callback:', insertError);
          }
        }
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        const isUuid = (id: string) => /^[0-9a-f]{8}-[0-9a-f]{4}-[0-4][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id);
        
        session.user.id = (token.supabaseId || (isUuid(token.id) ? token.id : null)) as string;
