...
BEGIN
    -- Wrap everything in exception handler to prevent user creation failures
    BEGIN
        ...
        IF NEW.organization_id IS NULL THEN
            ...
            IF pending_invitation.id IS NOT NULL THEN
                ...
                -- Cast enum to text for profiles.role (VARCHAR) column
                NEW.role := pending_invitation.role::text;
                ...
                EXCEPTION
                    WHEN others THEN
                        -- Log but don't fail
                        RAISE NOTICE 'Failed to add user to org members: %', SQLERRM;
                END;

                -- Log profile creation with invitation context (optional - don't fail if it errors)
                BEGIN
                    ...
                    VALUES (
                        ...
                        jsonb_build_object(
                            ...
                            'role', pending_invitation.role::text,
                            'via_invitation', true
                        )
                    );
                EXCEPTION
                    WHEN others THEN
                        NULL; -- Silently ignore activity logging errors
                END;
            ...
            EXCEPTION
                WHEN others THEN
                    RAISE NOTICE 'Failed to add user as admin member: %', SQLERRM;
            END;

            -- Log profile creation with new org (optional - don't fail if it errors)
            BEGIN
                INSERT INTO public.activity_log (user_id, action, entity_type, entity_id, metadata)
                VALUES (
                    NEW.id,
    ...
    EXCEPTION
        WHEN others THEN
            ...
            RAISE NOTICE 'Error in handle_new_profile_organization: %', SQLERRM;
            -- Still return NEW so the profile gets created
    END;
